...
# =============================================================================
# *The method works on the standard Gaussian space, transform your input random
...
"""
# ==========================================================================
def SG_local(dim, N, delta_star, g_LSF, I_smooth, grad_logI_smooth, refin, eps, se, maxit=30):
    np.random.seed(seed=se)

    # ==============================================
    # initialization of variables and storage
    samples = list()                  # store the samples
    LSF_eval = np.empty((maxit+1, N))   # space for the LSF evaluations
    ...
    def pi_rnd(N, d): return np.random.multivariate_normal(mean=np.zeros(d), cov=np.eye(d), size=N).T if (d > 0) else np.tile(np.array([]), [N, 1]).T
    def log_pi_pdf(x): return np.sum(sps.norm.logpdf(x, loc=0, scale=1), axis=0)
  
    # biasing parametric family (single Gaussian)
    def gbias_rnd(N, mu_hat, Sigma_hat): return np.random.multivariate_normal(mean=mu_hat, cov=Sigma_hat, size=N).T
    def log_gbias_fun(x, mu_hat, Sigma_hat): return sps.multivariate_normal.logpdf(x.T, mean=mu_hat, cov=Sigma_hat)
    ...
    while True:
        ...
        def fmin(s): return ((np.std(wIS_j*I_smooth(s, geval))/np.mean(wIS_j*I_smooth(s, geval))) - delta_star)**2
        if (j == 0):
        ...
        #
        if (np.isinf(H_hat).any() is True) or (np.sum(H_hat) <= 1e-5):
            ...
            eigval, eigvec = np.linalg.eigh(H_hat)   # np.linalg.eigh(H_hat)
        ...
        if (j > 0):   # store basis of the previous level
            Phi_r_old, Phi_p_old = Phi_r.copy(), Phi_p.copy()
        ...
        if (j > 0):
            ...
            new_Sigma = np.vstack([np.hstack([new_Sigma_hat_r, new_Sigma_hat_rp]),
                                   np.hstack([new_Sigma_hat_pr, new_Sigma_hat_p])])
