...
g_LSF      = lambda x: beta - np.sum(x, axis=0)/np.sqrt(dim)
grad_g_LSF = lambda x: -(1/np.sqrt(dim)) * np.ones((N,dim)) 

# choose smooth indicator function
f = 'tanh'
if (f == 'erf'):
    I_smooth         = lambda s, geval: 0.5 * (1 + sp.special.erf(-geval/(np.sqrt(2)*s)))#sps.norm.cdf(-geval/s)
