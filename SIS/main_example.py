...
burnin     = int(0.05*N)    # burnin
rho        = 0.1            # to define the number of seeds
...
for i in range(NSIM):  
    print('\n***Simulation ', i+1, '/', NSIM, '\n')
    Pf[i], s_param, samples, LSF_eval = SIS(dim, N, burnin, rho, delta_star, I_smooth, g_LSF, g_LSF_single, set_seed[i])
#
Pf_mean = np.mean(Pf)
Pf_std  = np.std(Pf)
